...
# pacman::p_load only installs if package is missing, then loads it
...
# Set working directory based on file location
current_file <- normalizePath(
  file.path(dirname(knitr::current_input()), "world-gov-indicators.qmd")
)
setwd(dirname(current_file))

# Get base SOURCE_REPO and combine with current directory
base_repo <- Sys.getenv("SOURCE_REPO")
if (base_repo == "") {
  # Fallback to default value if not set
  base_repo <- "https://github.com/renan-peres/website/blob/main/src"
  message("SOURCE_REPO not set, using default: ", base_repo)
}

# Extract relative path from current directory
relative_path <- "mban/quarto/world-gov-indicators"
source_repo <- file.path(base_repo, relative_path)

# Convert GitHub URL to raw content URL format
raw_url <- gsub(
...
knitr::opts_chunk$set(
  collapse = TRUE,
  echo = TRUE,
  fig.width = unit(7.78, "cm"),
  fig.height = unit(10, "cm")
)
...
**Last Update:** `r format(lubridate::now('America/New_York'), ' %B %d, %Y - %H:%M')`
...
// makeFullscreen("widgets/WGI_DT.html")
makeFullscreen(`${raw_url}/widgets/WGI_DT.html`)
```
...
// makeFullscreen("widgets/WGI_2021.html")
makeFullscreen(`${raw_url}/widgets/WGI_2021.html`)
```
...
top_3_plot <- df %>% 
                ...
                mutate(Country = as.character(Country),
                       Indicator = case_when(Indicator == "Government Effectiveness" ~ "Effectiveness",
                                             T ~ Indicator),
...
// makeFullscreen("widgets/Top_3_Comparison.html")
makeFullscreen(`${raw_url}/widgets/Top_3_Comparison.html`)
```
...
// makeFullscreen("widgets/USA_Plotly.html")
makeFullscreen(`${raw_url}/widgets/USA_Plotly.html`)
```
